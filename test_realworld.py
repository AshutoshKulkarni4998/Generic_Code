...
from utils import parse_args, RainDataset_REALWORLD, rgb_to_y, psnr, ssim, VGGPerceptualLoss
...
if __name__ == '__main__':
    args = parse_args()
    test_dataset = RainDataset_REALWORLD(args.data_path_test, args.data_path_test, args.data_name, 'test')
    ...
    #             train_dataset = RainDataset_REALWORLD(args.data_path, args.data_path_test, args.data_name, 'train', args.patch_size[i], length)
    #             train_loader = iter(DataLoader(train_dataset, args.batch_size[i], True, num_workers=args.workers))
    #             i += 1
    #         # train
    #         model.train()
    #         rain, name, h, w = next(train_loader)
    #         rain= rain.cuda()
    #         out = model(rain)
